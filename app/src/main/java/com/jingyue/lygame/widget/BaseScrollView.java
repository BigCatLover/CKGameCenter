...
//                contentView.layout(originalRect.left, originalRect.top, originalRect.right, originalRect.bottom);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                boolean shouldMove = (canPullDown && deltaY > 0) // 可以下拉， 并且手指向下移动
                        || (canPullUp && deltaY < 0) // 可以上拉， 并且手指向上移动; // 既可以上拉也可以下拉（这种情况出现在ScrollView包裹的控件比ScrollView还小）
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            if (getScrollY() > 150) {
                ...
                long del = Math.abs(now - lastSkipTime);
                if (skipListener != null && (lastSkipTime == 0 || del > 1000)) {
                    skipListener.onSkip();
                }
                lastSkipTime = now;
            }
