...
//		}
    ...
    private void buildCustomeShareParams(String name) {
        if (StringUtils.isEmptyOrNull(name) || null == shareParamsMap) {
            return;
        }
        ...
        if (name.equalsIgnoreCase(SinaWeibo.NAME)) {
            Object imageurl;
            Object url;
            if (shareParamsMap.containsKey("share_url_sina")) {
                url = shareParamsMap.get("share_url_sina");
            } else {
                url = shareParamsMap.get("url");
            }
            if (shareParamsMap.containsKey("imageurl_sina")) {
                imageurl = shareParamsMap.get("imageurl_sina");
            } else {
                imageurl = shareParamsMap.get("imageUrl");
            }

            //  Image2 + content +url     B
            Object content;
            if (shareParamsMap.containsKey("share_content_sina")) {
                content = shareParamsMap.get("share_content_sina");
            } else if (shareParamsMap.containsKey("title_sina")) {
                content = shareParamsMap.get("title_sina");
            } else {
                content = shareParamsMap.get("text");
            }

            Object text = null;
            if (null != content && !StringUtils.isEmptyOrNull(content.toString())) {
                text = content;
            }
            if (null != url && !StringUtils.isEmptyOrNull(url.toString())) {
                text = text + " " + url;
            }
            if (null != text && !StringUtils.isEmptyOrNull(text.toString())) {
                params.put("text", text);
            }

            if (null != imageurl && !StringUtils.isEmptyOrNull(imageurl.toString())) {
            ...
            Object jsParam = null;
            if (shareParamsMap.containsKey("jsParam_sina")) {
                jsParam = shareParamsMap.get("jsParam_sina");
            } else if (shareParamsMap.containsKey("jsParam")) {
                jsParam = shareParamsMap.get("jsParam");
            }
            ...
            Object imageUrl;
            Object url;
            if (shareParamsMap.containsKey("title_qq")) {
                title = shareParamsMap.get("title_qq");
            } else {
                title = shareParamsMap.get("title");
            }
            if (shareParamsMap.containsKey("share_content_qq")) {
                text = shareParamsMap.get("share_content_qq");
            } else {
                text = shareParamsMap.get("text");
            }
            if (shareParamsMap.containsKey("imageurl_qq")) {
                imageUrl = shareParamsMap.get("imageurl_qq");
            } else {
                imageUrl = shareParamsMap.get("imageUrl");
            }
            if (shareParamsMap.containsKey("share_url_qq")) {
                url = shareParamsMap.get("share_url_qq");
            } else {
                url = shareParamsMap.get("url");
            }
            if (null != title && !StringUtils.isEmptyOrNull(title.toString())) {
                // Image1 + title      B
                params.put("title", title);

            }
            if (null != text && !StringUtils.isEmptyOrNull(text.toString())) {
                // Image1 + content（title处无留白）     C
                params.put("text", text);
            }
            ...
            if (null != shareParamsMap.get("titleUrl") && !StringUtils.isEmptyOrNull(shareParamsMap.get("titleUrl").toString()))
                params.put("titleUrl", shareParamsMap.get("titleUrl"));

            if (null != url && !StringUtils.isEmptyOrNull(url.toString()))
                params.put("url", url);

            Object jsParam = null;
            if (shareParamsMap.containsKey("jsParam_qq")) {
                jsParam = shareParamsMap.get("jsParam_qq");
            } else if (shareParamsMap.containsKey("jsParam")) {
                jsParam = shareParamsMap.get("jsParam");
            }

        } else if (name.equalsIgnoreCase(QZone.NAME)) {
            //  QQ空间   Image1 + title +content      A QZone只有A

            Object title;
            Object text;
            Object imageUrl;
            Object url;
            if (shareParamsMap.containsKey("title_qzone")) {
                title = shareParamsMap.get("title_qzone");
            } else {
                title = shareParamsMap.get("title");
            }
            if (shareParamsMap.containsKey("share_content_qzone")) {
                text = shareParamsMap.get("share_content_qzone");
            } else {
                text = shareParamsMap.get("text");
            }
            if (shareParamsMap.containsKey("imageurl_qzone")) {
                imageUrl = shareParamsMap.get("imageurl_qzone");
            } else {
                imageUrl = shareParamsMap.get("imageUrl");
            }
            if (shareParamsMap.containsKey("share_url_qzone")) {
                url = shareParamsMap.get("share_url_qzone");
            } else {
                url = shareParamsMap.get("url");
            }
            ...
            if (null != shareParamsMap.get("site") && !StringUtils.isEmptyOrNull(shareParamsMap.get("site").toString()))
                params.put("site", shareParamsMap.get("site"));

            if (null != shareParamsMap.get("siteUrl") && !StringUtils.isEmptyOrNull(shareParamsMap.get("siteUrl").toString()))
                params.put("siteUrl", shareParamsMap.get("siteUrl"));

            Object jsParam = null;
            if (shareParamsMap.containsKey("jsParam_qzone")) {
                jsParam = shareParamsMap.get("jsParam_qzone");
            } else if (shareParamsMap.containsKey("jsParam")) {
                jsParam = shareParamsMap.get("jsParam");
            }
        }
        if (null != shareParamsMap && null != params) {
