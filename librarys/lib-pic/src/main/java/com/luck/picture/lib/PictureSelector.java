...
public final class PictureSelector {
     ...
     * @param medias
     */
    public void externalPicturePreview(int position, ArrayList<String> medias) {
        if (!DoubleUtils.isFastDoubleClick()) {
            Intent intent = new Intent(getActivity(), PictureExternalPreviewActivity.class);
            intent.putStringArrayListExtra(PictureConfig.EXTRA_PREVIEW_SELECT_LIST, medias);
            intent.putExtra(PictureConfig.EXTRA_POSITION, position);
            getActivity().startActivityForResult(intent, PictureConfig.PREVIEW_CODE);
