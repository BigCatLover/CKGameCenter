...
public class PtrFrameLayout extends ViewGroup {
    ...
    private static byte FLAG_HEADER_PIN_CONTENT = 0x01 << 3;
    private static byte FLAG_FOOTER_PIN_CONTENT = 0x01 << 4;
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mPtrIndicator.setInterpolator(decelerateInterpolator);
    ...
    private void layoutChildren() {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            PtrCLog.d(LOG_TAG, "onLayout offset: %s %s %s %s", offsetHeaderY, offsetFooterY, isHeaderPinContent(), mPtrIndicator.isHeader());
        }
        ...
        if (mContent != null) {
            ...
            if (mPtrIndicator.isHeader()) {
                ...
                top = paddingTop + lp.topMargin + (isHeaderPinContent() ? 0 : offsetHeaderY);
                right = left + mContent.getMeasuredWidth();
                bottom = top + mContent.getMeasuredHeight();
            } else {
                left = paddingLeft + lp.leftMargin;
                top = paddingTop + lp.topMargin - (isFooterPinContent() ? 0 : offsetFooterY);
                right = left + mContent.getMeasuredWidth();
                bottom = top + mContent.getMeasuredHeight();
            }
        ...
        if (mFooterView != null) {
            MarginLayoutParams lp = (MarginLayoutParams) mFooterView.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            final int top = paddingTop + lp.topMargin + contentBottom - (isHeaderPinContent() ? offsetFooterY : 0);
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //Log.e(LOG_TAG, String.format("ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s canHeaderMoveDown: %s canFooterMoveUp: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown, canHeaderMoveDown, canFooterMoveUp));
                ...
                if (canMoveUp) {
                    //Log.e("ptr", "start move then canMoveUp");
                    if (mStatus == PTR_STATUS_LOADING && isHeaderPinContent()) {
                        return dispatchTouchEventSupper(e);
                    }
                    moveHeaderPos(offsetY);
                    return true;
                ...
                if (canMoveDown) {
                    //Log.e("ptr", "start move then canMoveDown");
                    if (mStatus == PTR_STATUS_LOADING && isFooterPinContent()) {
                        return dispatchTouchEventSupper(e);
                    }
    ...
    private void updatePos(int change) {
        ...
        if (mPtrIndicator.isHeader()) {
            ...
            //非悬浮模式，位移会有问题。
            if (isPinContent()) {
                final int line = getMeasuredHeight() - mPtrIndicator.getOffsetToLoadMore();
                if (line < mFooterView.getTop()) {
                    if (mFooterView.getTop() - change <= line) {
                        change = mFooterView.getTop() - line;
                    }
    ...
    public boolean isPinContent() {
        return mPtrIndicator.isHeader() ? isHeaderPinContent() : isFooterPinContent();
    }

    public boolean isHeaderPinContent() {
        return (mFlag & FLAG_HEADER_PIN_CONTENT) > 0;
    }

    public boolean isFooterPinContent() {
        return (mFlag & FLAG_FOOTER_PIN_CONTENT) > 0;
    }

    /**
     * The content view will now move when pinContent set to true.
     */
    public void setPinContent(boolean headerPinContent, boolean footPinContent) {
        if (headerPinContent) {
            mFlag = mFlag | FLAG_HEADER_PIN_CONTENT;
        } else {
            mFlag = mFlag & ~FLAG_HEADER_PIN_CONTENT;
        }
        if (footPinContent) {
            mFlag = mFlag | FLAG_FOOTER_PIN_CONTENT;
        } else {
            mFlag = mFlag & ~FLAG_FOOTER_PIN_CONTENT;
        }
    }
