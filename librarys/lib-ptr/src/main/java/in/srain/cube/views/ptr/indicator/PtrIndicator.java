...
public class PtrIndicator {
    ...
    public float getOffsetY() {
        //return mOffsetY;
        return mOffsetY > 0 ? getHeaderOffsetY(mOffsetY) : getFooterOffsetY(mOffsetY);//mOffsetY;
    }

    private float getFooterOffsetY(float offsetY) {
        offsetY = Math.abs(offsetY);
        return -offsetY;
    }

    private float getHeaderOffsetY(float offsetY) {
        if (mInterpolator != null) {
            offsetY = mInterpolator.getInterpolation(offsetY / mOffsetToRefresh) * offsetY / 2;
        }
        return offsetY;
    }
